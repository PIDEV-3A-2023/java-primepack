...
public class GestionOperationController implements Initializable {
    ...
    private void trierParDate() {
        ...
        ObservableList<Operation> operations = TableView.getItems();

        // Trier la liste par nom
        TableView.getSortOrder().clear(); // Effacer tout tri précédent
        dateOperationCell.setSortType(TableColumn.SortType.ASCENDING); // Spécifier le type de tri (ascendant)
        TableView.getSortOrder().add(dateOperationCell); // Ajouter la colonne de tri
        TableView.sort(); // Appliquer le tri
    }
    ...
    private void LoadData() {
        ...
        refreshTable();
        animaleCell.setCellValueFactory(new PropertyValueFactory<>("NomAnimale"));
                    ...
                    if (empty) {
                        ...
                        deleteIcon.setOnMouseClicked((MouseEvent event) -> {
                            ...
                            if (alert.getResult() == ButtonType.YES) {
                                ...
                                tray.setMessage("supression succès");
    ...
    private void exportPDF(ActionEvent event) {
        try {
            ...
            String file_path = absolutePath + "\\src\\PDF\\PDF.pdf";
            List<Operation> list = new ArrayList<>(service.getAll());

            int firstRow = 700;
            PdfReader pdfReader = new PdfReader(file_path);

            PdfStamper pdfStamper = new PdfStamper(pdfReader, new FileOutputStream(absolutePath + "\\src\\PDF\\pdfmod.pdf"));
            ...
            File file = new File(absolutePath + "\\src\\PDF\\pdfmod.pdf");
