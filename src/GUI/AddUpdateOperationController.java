...
public class AddUpdateOperationController implements Initializable {
    ...
    @FXML
    private ComboBox<Animal> animeleCB;
    @FXML
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        try {
            listA.addAll(animalService.afficherListeA());
        } catch (SQLException ex) {
            Logger.getLogger(AddUpdateOperationController.class.getName()).log(Level.SEVERE, null, ex);
        }
        animeleCB.setItems(listA);
    ...
    private void ajoutOrDelete(ActionEvent event) {
        if (controleDeSaisie()) {
            if (type.equals("Update")) {
                ...
                operation.setAnimaleId(animeleCB.getSelectionModel().getSelectedItem().getId());
    ...
    public void initializeTextField(Operation o) {
        ...
        try {
            animeleCB.getSelectionModel().select(animalService.afficherA(o.getAnimaleId()));
        } catch (SQLException ex) {
            Logger.getLogger(AddUpdateOperationController.class.getName()).log(Level.SEVERE, null, ex);
        }
    ...
    private boolean controleDeSaisie() {
        ...
        if (animeleCB.getSelectionModel().getSelectedItem() == null) {

            TrayNotification tray = new TrayNotification();
            tray.setTitle("fail");
            tray.setMessage("Veuillez saisir l'animale");
            tray.setNotificationType(NotificationType.ERROR);
            tray.showAndDismiss(Duration.seconds(10));
            return false;
        }
