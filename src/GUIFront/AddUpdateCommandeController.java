...
public class AddUpdateCommandeController implements Initializable {
    ...
    private void ajoutOrDelete(ActionEvent event) {
        if (controleDeSaisie()) {
            if (type.equals("Update")) {
                ...
                commande.setIdUser(BaseController.currentUser.getId());
                update(commande);
            } else {
    ...
    private void ajout(Commande c) {
        ...
        try {
            ...
            p.setPrix((int) (c.getQuantite() * c.getProduit().getPrix()));
            Scene scene = new Scene(root);
            Stage stage = new Stage();
            stage.setScene(scene);
            stage.showAndWait();
            if (p.getReturn()) {
                commandeService.insert(c);
                Alert alert = new Alert(Alert.AlertType.INFORMATION);
            ...
            Logger.getLogger(AddUpdateCommandeController.class.getName()).log(Level.SEVERE, null, ex);
        }
